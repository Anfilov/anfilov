...
import {
  mapSanityToSluzbaData,
  type SanitySluzba,
} from "@/lib/sluzba-sanity-mapper";
...
export async function generateMetadata({
  ...
  const raw = (await getSluzbaBySlug(slug)) as SanitySluzba | null;
  if (!raw) return {};

  const sluzba = mapSanityToSluzbaData(raw);
  const url = `${siteConfig.url}/sluzba/${sluzba.slug}`;
  ...
  return {
    ...
    openGraph: {
      ...
      type: "website",
      images: [{ url: ogImage, width: 1200, height: 630, alt: sluzba.metaTitle }],
    },
    twitter: {
  ...
  const schemas: Record<string, unknown>[] = [
    {
      ...
      hasOfferCatalog: {
        ...
        itemListElement: sluzba.deliverables.map((d, i) => ({
          "@type": "Offer",
          itemOffered: { "@type": "Service", name: d.title, description: d.benefit },
          position: i + 1,
        })),
      },
    },
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 2, name: "Služby", item: `${siteConfig.url}/sluzba` },
        { "@type": "ListItem", position: 3, name: sluzba.name, item: url },
      ],
    },
  ];
